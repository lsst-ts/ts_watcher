...
from lsst.ts.watcher.rules import MTM2MirrorTemperature
from lsst.ts.xml.enums.Watcher import AlarmSeverity

STD_TIMEOUT = 5  # Max time to send/receive a topic (seconds)


class MTM2MirrorTemperatureTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_constructor(self) -> None:
        ...
        schema = watcher.rules.MTM2MirrorTemperature.get_schema()
        assert schema is not None

        rule = MTM2MirrorTemperature(None)
    ...
    async def test_operation(self) -> None:
        watcher_config_dict = dict(
            ...
            rules=[dict(classname="MTM2MirrorTemperature", configs=[{}])],
            escalation=(),
        )
        ...
        async with salobj.Controller("MTM2", 0) as controller, watcher.Model(
            ...
            rule_name = "MTM2MirrorTemperature.MTM2"
