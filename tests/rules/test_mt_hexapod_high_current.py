...
from lsst.ts.watcher.rules import MTHexapodHighCurrent
from lsst.ts.xml.enums.Watcher import AlarmSeverity

STD_TIMEOUT = 5  # Max time to send/receive a topic (seconds)


class MTHexapodHighCurrentTestCase(unittest.IsolatedAsyncioTestCase):

    def setUp(self):
        salobj.set_random_lsst_dds_partition_prefix()

    async def test_constructor(self):
        rule = MTHexapodHighCurrent(None)
    ...
    async def test_operation(self):
        watcher_config_dict = dict(
            ...
            rules=[dict(classname="MTHexapodHighCurrent", configs=[{}])],
            escalation=(),
        )
        ...
        async with salobj.Controller("MTHexapod", 1) as controller, watcher.Model(
            ...
            test_data_items = [
                {
                    "topic": controller.evt_highCurrent,
                    "highCurrent": True,
            ...
            rule_name = "MTHexapodHighCurrent.MTHexapod"
            ...
            for data_item in test_data_items:
                await watcher.write_and_wait(
                    model,
                    data_item["topic"],
                    highCurrent=data_item["highCurrent"],
                )
