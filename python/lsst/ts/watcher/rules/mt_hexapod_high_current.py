...
class MTHexapodHighCurrent(watcher.BaseRule):
    """Monitor the high motor current event of the MT hexapod.
    ...
    def __init__(
        ...
        remote_info_camera = RemoteInfo(
            remote_name,
            1,
            callback_names=["evt_highCurrent"],
        )
        ...
        super().__init__(
            config,
            f"MTHexapodHighCurrent.{remote_name}",
    ...
    def compute_alarm_severity(
        ...
        **kwargs : `dict` [`str`, `typing.Any`]
        ...
        exist_high_current = data.highCurrent

        return (
            (
                AlarmSeverity.WARNING,
                f"{sal_index.name} has the high motor current. Please consider"
                " to disable the CSC to cool down the motors.",
            )
            if exist_high_current
            else NoneNoReason
        )
