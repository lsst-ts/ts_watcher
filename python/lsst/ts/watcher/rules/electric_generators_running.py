...
class ElectricGeneratorRunning(BaseRule):
    ...
    def __init__(self, config, log=None):
        ...
        remote_info_list = [
            RemoteInfo(
                ...
                index=remote_index,
                callback_names=[],
                poll_names=["tel_agcGenset150"],
            )
        ]
        super().__init__(
            config=config,
            name=f"ElectricGeneratorRunning.{remote_name}",
    ...
    def compute_alarm_severity(
        ...
        **kwargs : `dict` [`str`, `typing.Any`]
        ...
        # Check for the running attribute
        genset_running = getattr(data, "running", None)
        if genset_running is None:
            return NoneNoReason
        reason = (
            f"{self._remote_name}:{self._remote_index}" " electric generator running."
        )
