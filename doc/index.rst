...
Each rule has one an associated `Alarm`, and is reponsible for determining the severity of that alarm.
The Model monitors state changes in alarms and issues ``alarm`` events as appropriate.

Rules may also emit ``notification`` events.
Notifications have no state; they cannot be acknowledged or muted.
It is recommended that each rule either determines the severity of its alarm, or emits ``notification`` events, not both.
